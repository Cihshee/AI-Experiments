...
class gui:
    ...
    def __init__(self, window):
        ...
        #设置标签
        L1 = tk.Label(window, text="请输入事实，最多4个！",
                 bg='yellow',font=('Arial', 12),width=25,height=2)
        L2 = tk.Label(window, text="推理过程与结果如下：",
                 bg='red',font=('Arial', 12),width=25,height=2)
        ...
        #设置按钮
        self.b1 = tk.Button(window,text="开始推理",
                            width=15,height=2,
    ...
    def add_rule_gui(self):
        ...
        l2 = tk.Label(new_win, text="Size(体型):",
                 bg='red',font=('Arial', 12),width=25,height=2)
        l2.grid(row=1,column=0,padx=10,pady=10)
        l3 = tk.Label(new_win, text="Character(特点):",
                 bg='red',font=('Arial', 12),width=25,height=2)
        l3.grid(row=2, column=0, padx=10, pady=10)
        l4 = tk.Label(new_win, text="Vehicle(交通工具种类):",
                 bg='red',font=('Arial', 12),width=25,height=2)
    ...
    def add_rule(self, t1,t2,t3,t4):
        ...
        # SQL 插入语句
        sql = "INSERT INTO RULE(ID, SIZE, \
            CHARACTERS,VEHICLE,RESULTS) \
               VALUES (%d,'%s','%s','%s','%s')" % \
              (max_id, t1,t2,t3,t4)
        try:
            # print(sql)
    ...
    def update_gui(self):
        ...
        l1 = tk.Label(new_win, text="请输入您想修改的规则编号（ID）：",
                      bg='red', font=('Arial', 12), width=30, height=2)
        l1.grid(row=0, column=0, padx=10, pady=10)
        l2 = tk.Label(new_win, text="Size(体型):",
                      bg='red', font=('Arial', 12), width=30, height=2)
        l2.grid(row=1, column=0, padx=10, pady=10)
        ...
        l5 = tk.Label(new_win, text="Result(结果):",
                      bg='red', font=('Arial', 12), width=30, height=2)
        l5.grid(row=4, column=0, padx=10, pady=10)
        l6 = tk.Label(new_win, text="提醒：若有空不填，则会修改对应项为空！！！",
                      bg='red', font=('Arial', 12), width=40, height=2)
    ...
    def matching(self):
        ...
        for i in range(4):
            if str[i] != '':
                sql = "INSERT INTO SYNTHESIS(ID, FACTS) \
                                       VALUES (%d,'%s')" % \
                      (ID, str[i])
                try:
                    cursor.execute(sql)
